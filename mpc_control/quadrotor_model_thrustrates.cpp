...
int main( ){
  ...
  */
  ...
  Control               T, w_x, w_y, w_z;  // Body rate control
  ...
  const double w_max_yaw = 0.3;     // Maximal yaw rate [rad/s]
  const double w_max_xy = 6.28;      // Maximal pitch and roll rate [rad/s]
  const double T_min = 0.0;      // Minimal thrust [mg] unit is mass in Newton
  const double T_max = 2.0 * g_z;        // Maximal thrust [mg]
  ...
  f << dot(v_x) ==  - 2 * ( q_w * q_y + q_x * q_z ) * T; // How easy can one add drag?
  ...
  ocp.subjectTo( T+(w_x+w_y)/3.14 <= T_max);
